        ...
        ws.onmessage = (event) => {
            ...
            switch (data.type) {
                ...
                case 'result':
                    ...
                    // Store initial data from WebSocket
                    ...
                    // Fetch full session details to get rich media (youtube embeddings, etc)
                    console.log('Fetching full research session details for:', sessionId)
                    fetch(`http://localhost:8000/api/research/sessions/${sessionId}`)
                        .then(res => res.json())
                        .then(sessionData => {
                            if (sessionData.success && sessionData.research) {
                                console.log('Rich research data received:', sessionData.research)

                                // Update state with rich data
                                if (sessionData.research.metadata) {
                                    const meta = sessionData.research.metadata

                                    // Update images if available
                                    if (meta.images && Array.isArray(meta.images)) {
                                        setResearchImages(meta.images)
                                    }

                                    // Update metadata (contains youtube videos)
                                    setResearchMetadata(meta)

                                    // Update news if available
                                    if (meta.news) {
                                        setResearchNews(meta.news)
                                    }
                                }
                            }
                        })
                        .catch(err => {
                            console.error('Failed to fetch rich research session details:', err)
                        })
                    break
